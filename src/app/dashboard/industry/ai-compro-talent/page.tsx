...
export default function AiComproTalentPage() {
  ...
  // Premium state (default check: if super_admin always premium, or can toggle demo)
  const isSuperAdmin = role === "super_admin";
  const [isPremiumUser, setIsPremiumUser] = useState<boolean>(true); // Default true or demo accessible
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] p-4 lg:p-8 space-y-6">
      ...
      {/* Modal: Upgrade Premium Alert / Paywall */}
      {showPremiumModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-xs p-4">
          <div className="bg-white rounded-3xl p-7 max-w-lg w-full shadow-2xl border border-gray-100 space-y-5 text-center">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-amber-400 to-amber-600 text-white flex items-center justify-center mx-auto shadow-md">
              <Crown className="w-8 h-8" />
            </div>

            <div className="space-y-1.5">
              <h3 className="text-xl font-black text-gray-900">
                Fitur Eksklusif Perusahaan Premium
              </h3>
              <p className="text-xs text-gray-500 max-w-sm mx-auto">
                Analisis Company Profile otomatis dan pencarian ribuan talenta muda SMK & Mahasiswa berprestasi hanya tersedia untuk mitra industri Premium.
              </p>
            </div>

            <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100 text-left space-y-2.5 text-xs text-gray-700">
              <div className="flex items-center gap-2.5">
                <Sparkles className="w-4 h-4 text-[#035a70] flex-shrink-0" />
                <span>Ekstraksi kebutuhan kompetensi industri otomatis oleh Gemini AI</span>
              </div>
              <div className="flex items-center gap-2.5">
                <CheckCircle2 className="w-4 h-4 text-emerald-600 flex-shrink-0" />
                <span>Skor kesesuaian talent matching akurat hingga 95%+</span>
              </div>
              <div className="flex items-center gap-2.5">
                <Send className="w-4 h-4 text-blue-600 flex-shrink-0" />
                <span>Akses direct kontak (WhatsApp, Email) dan kirim undangan seleksi langsung</span>
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
              <button
                ...
                className="w-full sm:w-1/2 py-2.5 text-xs font-semibold text-gray-600 hover:text-gray-800 bg-gray-100 hover:bg-gray-200 rounded-xl transition-colors"
              >
                Tutup
              </button>
              <Link
                href="/dashboard/subscription-tiers"
                className="w-full sm:w-1/2 py-2.5 text-xs font-bold text-white bg-[#035a70] hover:bg-[#024353] rounded-xl shadow-md flex items-center justify-center gap-1.5 transition-all"
              >
